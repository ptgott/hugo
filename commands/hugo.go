...
func (c *commandeer) fullRebuild(changeType string) {
	...
	go func() {
		...
		// Preserve the previous Hugo state so we can roll back the new state
		// if we fail to reload the user's configuration.
		c.created = false
		p := c.commandeerHugoState
		n := newCommandeerHugoState()
		fmt.Println("TESTDEBUG CALLING LOADCONFIG")
		err := c.loadConfig()
		if err != nil {
			// Set the processing on pause until the state is recovered.
			c.paused = true
			c.commandeerHugoState = p
